...
import ViewUser from "../Owner/ViewUser";
...
export default function DistributorMedicineInfo(props) {
  ...
  //const { name, locationx, locationy, role } = props.location.query.userInfo;
  ...
  async function getMedicineData() {
    ...
    let mdata=await medicine.methods.getMedicineInfo().call({ from: account });
    ...
    console.log(status);
    let  customLink =`https://ab33-223-233-87-85.ngrok-free.app/distributor/view-medicine/${medicineAddress}`;
    if (status === 2) {
      activeStep = 3;
    } else if (status === 3) {
      activeStep = 2;
